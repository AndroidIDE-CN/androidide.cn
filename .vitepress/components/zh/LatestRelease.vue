<template>
  <div class="release-container" v-if="release">
    <div class="release-header">
      ...
      <p class="release-date"><strong>提交时间:</strong> {{ new Date(release.published_at).toLocaleDateString() }}</p>
    </div>
    <div class="release-body" v-if="release.body" v-html="release.body.replace(/\r?\n/g, '<br>')"></div>
    <div class="release-body" v-else>没有提交信息.</div>
    ...
    <mdui-button variant="tonal" :href="release.html_url" target="_blank" full-width>在Github中打开</mdui-button>
  </div>
  <div v-else>
    加载中...
  </div>
